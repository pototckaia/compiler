...
void AsmGenerator::visit(MainFunction& m) {
  ...
  for (auto& var : m.getDecl().tableVariable) {
    var.second->accept(a);
  }

  // set label for function
  for (auto& var : m.getDecl().tableFunction) {
    auto label = getLabelName(var.second->getName());
    var.second->setLabel(label);
  }

  stackTable.push(m.getDecl());
  asm_file
    << cmd(code)
  ...
  // function decl
  for (auto& fun : m.getDecl().tableFunction) {
...
void AsmGenerator::visit_function(SymFun& fun) {
  ...
  for (auto& e : tableLocal.tableVariable) {
    ...
    if (!s->isProcedure() && e.first == fun.getName()) { // result var
  ...
  asm_file
    << Comment("Function call " + fun.getName())
  ...
  for (auto& e : s->getParamList()) {
    if (e->getType()->isOpenArray() && e->getSpec()== ParamSpec::NotSpec) {
      auto array = std::dynamic_pointer_cast<OpenArray>(e->getType());
...
void AsmGenerator::visit(Variable& v) {
  if (v.getType()->isProcedureType() && 
      stackTable.isFunction(v.getVarName())) {
    stackTable.findFunction(v.getVarName())->accept(*this);
  } else {
    stackTable.findVar(v.getVarName())->accept(*this);
  }
  asm_file
...
void AsmGenerator::visit(Literal& l) {
  ...
  } else if (l.getType()->isDouble()) {
    asm_file
      << Comment("double literal")
      << cmd(MOV, {RAX}, {l.getValue().getDouble(), double64})
      << cmd(PUSH, {RAX});
  } else if (l.getType()->isChar()) {
    asm_file
      << Comment("char literal")
      << cmd(PUSH, {l.getVarName()});
  } else if (l.getType()->isString()) {
    auto label_name = add_string(l.getVarName());
    asm_file
...
void AsmGenerator::visit_cmp(BinaryOperation& b) {
  ...
  if (b.getRight()->getType()->isDouble()) {
    asm_file
      << cmd(POP, {R11}) // right
      << cmd(POP, {RAX}) // left
      << cmd(MOVQ, {XMM0, none}, {RAX})
...
void AsmGenerator::visit(OpenArray& o) {
  ...
  asm_file
    ...
    << cmd(PUSH, {RCX}); // new base = base + index*sizeof
  bounds.pop_front();
  bounds.front()->getType()->accept(*this);
}

void AsmGenerator::visit(ArrayAccess& a) {
  bool lvalue = need_lvalue;
  if (a.getName()->getType()->isPointer()) {
    need_lvalue = false;
    a.getName()->accept(*this);
  } else {
    visit_lvalue(*a.getName());
  }
  bounds = std::move(a.getListIndex());
  a.getName()->getType()->accept(*this);
  asm_file
...
void AsmGenerator::visit(RecordAccess& r) {
  bool lvalue = need_lvalue;
  visit_lvalue(*r.getRecord());
  auto record = r.getRecord()->getType()->getRecord();
  ...
  asm_file
    ...
    << cmd(PUSH, {R8});
  if (!lvalue) {
    pushRvalue(r.getType());
  }
}

void AsmGenerator::visit(Cast& c) {
  if (need_lvalue) {
    visit_lvalue(*c.getExpr());
  } else {
    c.getExpr()->accept(*this);
  }

  if (c.getExpr()->getType()->isDouble() && c.getType()->isInt()) {
    asm_file
      ...
      << cmd(PUSH, {RAX});
  } else if (c.getExpr()->getType()->isInt() && c.getType()->isDouble()) {
    asm_file
...
void AsmGenerator::visit(AssignmentStmt& a) {
  ...
  visit_lvalue(*a.getLeft());
  if (a.getLeft()->getType()->isTrivial()) {
  ...
  // count 8 byte
  uint64_t len = a.getLeft()->getType()->size() / 8;
  auto _start = getLabel();
  auto _end = getLabel();
  asm_file
  ...
  if (!e.getReturnType()->isVoid()) {
    AssignmentStmt c( // result := expr;
      ...
      std::make_unique<Variable>(
        Token(-1, -1, TokenType::String, e.assignmentVar->name, e.assignmentVar->name),
        e.getReturnType()),
  ...
  for (auto& e : params) {
    ...
    if (e->getType()->isString()) {
      asm_file
        ...
        << cmd(XOR, {RAX}, {RAX})
        << cmd(CALL, {Printf});
    } else if (e->getType()->isInt() || e->getType()->isPointer()) {
      asm_file
        << Comment("printf int")
...
void AsmGenerator::visit(FunctionCall& f) {
  ...
  if (f.getName()->getEmbeddedFunction() != nullptr) {
    syscall_params = std::move(f.getParam());
    f.getName()->getEmbeddedFunction()->accept(*this);
    return;
  } else {
    auto s = f.getName()->getType()->getSignature();
    ...
    for (auto iterArgs = f.getParam().begin();
         iterArgs != f.getParam().end(); ++iterArgs, ++iterParams) {
      if ((*iterParams)->getType()->isOpenArray()) {
        auto array = (*iterArgs)->getType()->getStaticArray();
        asm_file
    ...
    if (stackTable.isFunction(f.getName()->getType()->getName())) {
      visit_lvalue(*f.getName());
    } else {
...
void AsmGlobalDecl::visit(GlobalVar& v) {
  v.setLabel(getLabelName(v.name));
  a
    << cmd(bss)
    << Label(v.label) << ": ";
...
void AsmGlobalDecl::visit(Alias& a) { a.getType()->accept(*this); }
